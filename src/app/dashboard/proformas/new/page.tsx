...
export default function NewProformaPage() {
  ...
  const [proformaNumber, setProformaNumber] = useState("003-001-000000001");
  ...
  useEffect(() => {
    if (!db) return;
    const q = query(collection(db, "proformas"), orderBy("proformaNumber", "desc"), limit(1));
    const unsubscribe = onSnapshot(q, (snapshot) => {
      if (!snapshot.empty) {
        const lastDoc = snapshot.docs[0].data();
        const lastNum = lastDoc.proformaNumber || "003-001-000000000";
        const parts = lastNum.split("-");
        if (parts.length === 3) {
          const sequence = parseInt(parts[2]) + 1;
          setProformaNumber(`003-001-${sequence.toString().padStart(9, '0')}`);
        }
      } else {
        setProformaNumber("003-001-000000001");
      }
    });
    return () => unsubscribe();
  }, [db]);
  ...
  const handleSaveCustomerToDirectory = async () => {
    if (!db || !clientData.ruc || !clientData.name) {
      toast({ title: "Datos incompletos", description: "Identificación y Nombre son obligatorios.", variant: "destructive" });
      return;
    }
    ...
    try {
      ...
      if (snap.empty) {
        ...
        toast({ title: "Cliente guardado en el directorio" });
      } else {
  ...
  const handleSave = async (customStatus?: string) => {
    if (!db) return null;
    if (isConsumidorFinal && totalWithIVA > 50) { setShowLimitModal(true); return null; }
    if (!clientData.name || (!isConsumidorFinal && clientData.ruc.length < 10)) { toast({ title: "Datos incompletos", variant: "destructive" }); return null; }
    ...
    try {
      ...
      if (savedDocId) await updateDoc(doc(db, "proformas", savedDocId), proformaData);
      else {
        proformaData.createdAt = serverTimestamp();
      ...
      return currentId;
    } catch (e) { toast({ title: "Error", variant: "destructive" }); return null; }
  ...
  const handleGeneratePDF = () => {
    ...
    try {
      generateBillingPDF({ title: "Proforma", client: clientData, items: items.filter(i => i.description.trim() !== ""), subtotal: subtotalBase, iva: ivaCalculated, total: totalWithIVA, deposit, balance, date: format(date, "dd/MM/yyyy"), docNumber: proformaNumber, observations, color: [79, 70, 229] });
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8 p-6 animate-in fade-in duration-500">
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 sticky top-0 z-30 bg-background/95 backdrop-blur-lg py-4 border-b rounded-xl px-4">
        <div className="flex items-center gap-4">
          <Button variant="ghost" size="icon" onClick={() => router.back()}><ArrowLeft className="h-5 w-5" /></Button>
          <h1 className="text-3xl font-black tracking-tighter">Nueva Proforma</h1>
        ...
        <div className="lg:col-span-2 space-y-8">
          <Card className="border-none shadow-sm bg-white rounded-xl p-8 space-y-8">
            <div className="flex items-center justify-between border-b pb-4">
              ...
              <div className="flex gap-2">
                ...
                <Button 
                  variant="ghost" 
                  size="sm" 
                  className="h-8 text-[10px] font-black uppercase text-indigo-600 hover:text-indigo-700"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
              ...
              <div className="space-y-2"><Label className="font-bold text-slate-700 uppercase text-[10px]">Teléfono:</Label><Input placeholder="0998765432" value={clientData.phone} onChange={(e) => setClientData({...clientData, phone: e.target.value.replace(/\D/g, '')})} className="bg-slate-50 h-11" /></div>
              <div className="space-y-2 md:col-span-2"><Label className="font-bold text-slate-700 uppercase text-[10px]">Dirección:</Label><Input value={clientData.address} onChange={(e) => setClientData({...clientData, address: e.target.value})} className="bg-slate-50 h-11" disabled={isConsumidorFinal} /></div>
            </div>
          </Card>
          <Card className="border-none shadow-sm bg-white rounded-xl p-8 mt-8">
            <Label className="font-bold text-slate-700 uppercase text-[10px]">Observaciones / Términos:</Label>
            <Textarea placeholder="Añada validez de la oferta, tiempos de entrega, etc." value={observations} onChange={(e) => setObservations(e.target.value)} className="bg-slate-50 border-slate-200 rounded-xl min-h-[100px] mt-2" />
          </Card>
        </div>
      ...
      <div className="col-span-full mt-8">
        <Card className="border-none shadow-xl bg-white rounded-3xl p-8">
          <h3 className="text-xl font-bold mb-6 border-b pb-4">Detalle de Proforma</h3>
          <div className="space-y-4">
            {items.map((item) => (
              <div key={item.id} className="p-6 bg-slate-50/50 rounded-2xl border flex flex-col md:flex-row gap-6">
                    ...
                    <PopoverContent className="p-0 w-[450px]">
                      ...
                      <div className="max-h-[300px] overflow-y-auto">
                        {filteredProducts.map((p: any) => (
                          <button key={p.id} className="w-full text-left px-4 py-3 hover:bg-indigo-50 flex justify-between border-b items-center" onClick={() => { setItems(items.map(i => i.id === item.id ? { ...i, description: p.name, unitPrice: p.price, productId: p.id, maxStock: p.stock !== undefined ? p.stock : null } : i)); setOpenPopoverId(null); }}>
                            ...
                            <div className="flex gap-2 items-center">
                              ...
                              <span className="text-indigo-600 font-black">${p.price.toFixed(2)}</span>
                            </div>
                          </button>
                        ))}
                      </div>
                ...
                <div className="w-32 space-y-2"><Label className="text-[10px] uppercase font-bold text-slate-400">P. Unitario</Label><Input type="number" value={item.unitPrice} onChange={(e) => setItems(items.map(i => i.id === item.id ? { ...i, unitPrice: parseFloat(e.target.value) || 0 } : i))} className="h-11 text-right font-black" /></div>
          ...
          <Button onClick={() => setItems([...items, { id: Math.random().toString(36).substr(2, 9), description: "", quantity: 1, unitPrice: 0, productId: null, maxStock: null }])} variant="outline" className="mt-6 border-dashed border-2 w-full h-14 font-bold text-indigo-600"><Plus className="mr-2 h-4 w-4" /> Añadir Item</Button>
        </Card>
      </div>
      ...
      <div className="flex justify-end mt-8">
        <Card className="bg-indigo-600 text-white rounded-3xl p-8 w-full md:w-96 shadow-xl space-y-6">
          <div className="space-y-3">
            <div className="flex justify-between text-sm opacity-90"><span>Subtotal (IVA 0%):</span><span>${subtotalBase.toFixed(2)}</span></div>
            <div className="flex justify-between text-sm opacity-90"><span>IVA 0%:</span><span>$0.00</span></div>
            <div className="pt-3 border-t border-white/20 flex justify-between items-end">
              ...
              <span className="text-4xl font-black">${totalWithIVA.toFixed(2)}</span>
            </div>
          </div>
      ...
      <AlertDialog open={showLimitModal} onOpenChange={setShowLimitModal}><AlertDialogContent className="rounded-2xl"><AlertDialogHeader><AlertDialogTitle className="text-center text-rose-700 font-black">Límite Superado</AlertDialogTitle><AlertDialogDescription className="text-center">Consumidor Final no puede exceder $50.00.</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter className="justify-center"><AlertDialogAction className="bg-rose-600" onClick={() => setShowLimitModal(false)}>Entendido</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog>
    </div>
  );
}
